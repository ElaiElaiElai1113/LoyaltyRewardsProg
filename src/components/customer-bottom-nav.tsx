...
export function CustomerBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card/95 px-3 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 shadow-card backdrop-blur md:hidden">
          ...
          return (
            <NavLink
              ...
              className={cn(
                ...
                isActive
                  ? 'bg-muted text-foreground after:absolute after:inset-x-4 after:top-1 after:h-0.5 after:rounded-full after:bg-[var(--accent-gold)]'
            ...
            >
              <span className="relative">
                ...
                {item.to === '/cart' && cartCount > 0 ? (
                  <span className="absolute -right-2 -top-2 flex min-w-4 items-center justify-center rounded bg-primary px-1 text-[0.6rem] font-bold leading-4 text-primary-foreground">
                    {cartCount}
                  </span>
                ) : null}
              </span>
