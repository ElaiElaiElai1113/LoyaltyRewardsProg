    ...
    <div className={cn('flex items-center gap-2', className)}>
      ...
      <Select value={language} onValueChange={(value) => setLanguage(value as Language)}>
        <SelectTrigger
          ...
          className={cn(
            'h-10 border-primary-container/25 bg-primary-container/8 text-xs font-bold uppercase tracking-[0.08em] text-current shadow-none focus-visible:ring-primary-container/40',
            compact ? 'w-[5.25rem] px-3' : 'w-[8.75rem]',
          )}
        >
          <SelectValue />
        </SelectTrigger>
