    ...
    <div className="space-y-10">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-4">
          <h1 className="font-display text-5xl tracking-tight text-primary">Gift Cards</h1>
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-card shadow-card p-6">
          <p className="text-sm text-on-surface-variant">Catalog Items</p>
          <p className="mt-2 font-display text-4xl text-primary-container">{catalog.data?.length ?? 0}</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-card shadow-card p-6">
          <p className="text-sm text-on-surface-variant">Issued Cards</p>
          <p className="mt-2 font-display text-4xl text-primary-container">{giftCards.data?.length ?? 0}</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-card shadow-card p-6">
          <p className="text-sm text-on-surface-variant">Active Cards</p>
          <p className="mt-2 font-display text-4xl text-primary-container">
      ...
      <section className="space-y-4">
        <h2 className="font-display text-3xl text-primary-container">Catalog</h2>
        <div className="grid gap-4">
          {(catalog.data ?? []).map((item) => (
            <div key={item.id} className="rounded-xl border border-[var(--border)] bg-card shadow-card flex items-center justify-between gap-4 p-5">
              <div>
                <h3 className="font-display text-2xl text-primary-container">{item.title}</h3>
      ...
      {businessId ? (
        <section className="space-y-4">
          <h2 className="font-display text-3xl text-primary-container">Issued</h2>
          <div className="grid gap-4">
            {(giftCards.data ?? []).map((card) => (
              <div key={card.id} className="rounded-xl border border-[var(--border)] bg-card shadow-card flex items-center justify-between gap-4 p-5">
                <div>
                  <h3 className="font-display text-2xl text-primary-container">{card.catalog?.title ?? card.code}</h3>
