...
export function QrScanner({ onDetected }: QrScannerProps) {
  ...
  const [message, setMessage] = useState('Point the device camera at a QR code or upload a screenshot.')
  ...
  function handleDetected(value: string) {
    setMessage('QR detected. Review the result before redeeming.')
    stopCamera()
    onDetected(value)
  }
  ...
  async function startCamera() {
    ...
    if (!navigator.mediaDevices?.getUserMedia || !BarcodeDetector) {
      setMessage('Live camera scanning is not available in this browser. Use upload or paste the code.')
      return
    }
