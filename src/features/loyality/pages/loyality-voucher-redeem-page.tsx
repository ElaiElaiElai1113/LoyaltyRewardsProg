...
export function LoyalityVoucherRedeemPage() {
  ...
  return <div className="mx-auto max-w-2xl"><section className="overflow-hidden rounded-[2rem] border border-[var(--border)] bg-card shadow-xl"><div className="bg-[#1f3a2e] p-7 text-white sm:p-10"><div className="flex items-center gap-3"><ShieldCheck className="text-[#d8b36a]" /><span className="text-xs font-black uppercase tracking-[.2em]">Staff voucher check</span></div><h1 className="mt-5 text-4xl font-black tracking-[-.05em] sm:text-6xl">{voucher.title}</h1><p className="mt-4 text-white/70">{voucher.description}</p></div><div className="p-6 sm:p-9"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[var(--muted)] p-5"><span className="text-xs font-black uppercase tracking-wider text-[var(--muted-foreground)]">Voucher value</span><strong className="mt-2 block text-2xl">{valueLabel(voucher, program.currency, program.locale)}</strong></div><div className="rounded-2xl bg-[var(--muted)] p-5"><span className="text-xs font-black uppercase tracking-wider text-[var(--muted-foreground)]">Status</span><strong className="mt-2 block text-2xl capitalize">{voucher.status}</strong></div></div>{voucher.status === 'active' ? <Button className="mt-6 h-14 w-full rounded-full bg-[#b8862e] text-base font-black text-white" onClick={() => void redeem()} disabled={redeeming}>{redeeming ? <LoaderCircle className="animate-spin" /> : <Gift />}Confirm voucher used</Button> : <div className="mt-6 rounded-3xl border border-[#466c55]/20 bg-[#d8b36a]/20 p-5 text-center"><CheckCircle2 className="mx-auto text-[#466c55]" /><strong className="mt-2 block">This voucher has already been recorded as used.</strong></div>}<p className="mt-5 text-center text-xs text-[var(--muted-foreground)]">Redeeming changes only this voucher. It does not combine with cash or a POS balance.</p></div></section></div>
}
