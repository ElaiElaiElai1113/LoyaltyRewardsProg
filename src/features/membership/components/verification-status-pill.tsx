...
export function VerificationStatusPill({ status }: VerificationStatusPillProps) {
  const { t } = useLanguage()
  ...
  return (
    <Link
      to={content.to}
      className={`hidden items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-bold transition hover:opacity-85 sm:inline-flex ${content.className}`}
    >
