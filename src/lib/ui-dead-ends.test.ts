...
describe('current UI process safeguards', () => {
  ...
  it('uses tenant commerce data instead of cross-tenant currency defaults', () => {
    ...
    expect(home).toContain('const homepageValueItems = isPinasRewards ? valueItems : tenantManagedValueItems')
    expect(home).toContain('const homepageProcessSteps = isPinasRewards ? processSteps : tenantManagedProcessSteps')
    expect(home).toContain('const homepageFaqs = isPinasRewards ? pinasRewardsFaqs : tenantManagedFaqs')
    expect(home).toContain('within Pinas Rewards—not cash exchange.')
