...
describe('RewardMe pitch alignment', () => {
  ...
  it('gives the pinas tenant its supplied RewardMe HTML landing experience', () => {
    ...
    expect(rewardMeHome).toContain('One account. Clear offers. Local rewards.')
    expect(rewardMeHome).toContain('The offer tells you exactly what you can earn.')
    expect(rewardMeHome).toContain('Rewards are program value, not cash.')
    expect(rewardMeHome).not.toContain('Planned savings options')
    expect(rewardMeHome).not.toContain('Approval required')
    expect(rewardMeHome).toContain('Reference price. Manual activation')
  ...
  it('keeps trial and manual-enrollment claims honest across signup and membership', () => {
    ...
    expect(join).toContain('Regular and Gold are requests only')
