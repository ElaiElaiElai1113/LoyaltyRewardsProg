...
describe('agreement Edge Functions', () => {
  ...
  it('sign-agreement verifies the caller and stores audit metadata', () => {
    ...
    expect(signAgreementFunction).toContain(".from('program_memberships')")
    expect(signAgreementFunction).toContain(".eq('program_id', payload.programId)")
    expect(signAgreementFunction).toContain('Active program membership is required.')
