...
test.describe('platform guide workflow', () => {
  ...
  test('public guide follows the English language preference without Spanish guide copy', async ({ page }) => {
    ...
    await expect(page.getByText('English script')).toBeVisible()
    await expect(page.getByText('Recording script')).toBeVisible()
    await expect(page.getByText('Screen storyboard')).toBeVisible()
    await expect(page.locator('body')).toContainText('What the platform does')
    await expect(page.locator('body')).toContainText('Customer flow')
    await expect(page.locator('body')).toContainText('Business flow')
    await expect(page.locator('body')).toContainText('Admin flow')
