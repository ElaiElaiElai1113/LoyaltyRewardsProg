...
runTest('tenant logo assets stay available while install metadata remains host aware', () => {
  ...
  assert.doesNotMatch(brandLogo, /\/medellin-rewards-logo\.svg/)
})
...
runTest('member signup schema no longer requires ID verification fields', () => {
  ...
  assert.match(authService, /phone,[\s\S]*active_program_id: getActiveProgram\(\)\.id/)
...
runTest('signup referrals are validated before Auth creates an account', () => {
  const forms = readFileSync('src/types/forms.ts', 'utf8')
  const authService = readFileSync('src/integrations/supabase/services/auth-service.ts', 'utf8')
  const authProvider = readFileSync('src/features/auth/auth-provider.tsx', 'utf8')
  const validationApi = readFileSync('api/validate-referral.ts', 'utf8')
  const joinPage = readFileSync('src/features/join/pages/join-rewards-page.tsx', 'utf8')
  const referralPage = readFileSync('src/features/referrals/pages/referral-register-page.tsx', 'utf8')
  const migration = readFileSync('supabase/migrations/20260902164106_enforce_valid_signup_referrals.sql', 'utf8')
  const compatibilityFix = readFileSync('supabase/migrations/20260902165542_fix_signup_referral_coalesce.sql', 'utf8')

  assert.match(forms, /referralCode\?: string \| null/)
  assert.match(authService, /await validateReferralBeforeSignUp\(input\)/)
  assert.match(authService, /fetch\('\/api\/validate-referral'/)
  assert.match(authService, /referral_code: referral\.referralCode/)
  assert.match(validationApi, /supabase\.rpc\('validate_signup_referral'/)
  assert.match(validationApi, /SUPABASE_SERVICE_ROLE_KEY/)
  assert.match(authProvider, /createPendingReferralForProfile\(sessionProfile, values\)/)
  assert.doesNotMatch(authProvider, /sessionStorage\.getItem\('referralCode'\)/)
  assert.match(joinPage, /referralCode: code \|\| null/)
  assert.match(joinPage, /id="join-referral-error" role="alert"/)
  assert.match(referralPage, /referralCode: referrerId/)
  assert.match(referralPage, /partnerReferralCode: partnerCode/)
  assert.match(migration, /before insert on auth\.users/)
  assert.match(migration, /message = 'invalid_referral_code'/)
  assert.match(migration, /grant execute on function public\.validate_signup_referral[\s\S]*to service_role/)
  assert.doesNotMatch(migration, /to anon, authenticated/)
  assert.match(compatibilityFix, /pg_catalog\.btrim\(coalesce\(p_referral_code, ''\)\)/)
  assert.doesNotMatch(compatibilityFix, /pg_catalog\.coalesce/)
})
...
runTest('landing Join CTAs go to customer account creation', () => {
  ...
  assert.match(landingPage, /Join \{programName\}/)
  assert.match(landingPage, /replaceAll\('\{programName\}', program\.name\)/)
  assert.match(landingPage, /Regular Membership/)
...
runTest('early access API creates leads through Supabase server-side', () => {
  ...
  assert.match(api, /rpc\('resolve_program_by_hostname'/)
  assert.match(api, /rpc\('create_program_early_access_lead'/)
...
runTest('welcome email API uses server-only Hostinger SMTP settings', () => {
  const api = readFileSync('api/send-welcome-email.ts', 'utf8')
  const templates = readFileSync('api/_tenant-email-templates.ts', 'utf8')
  ...
  assert.match(templates, /password-recovery/)
  assert.match(templates, /email-verification/)
  assert.match(templates, /administrator-invitation/)
  assert.match(templates, /invitations, rewards opportunities, and member updates/)
...
runTest('Figma homepage uses the approved typography and clean photography assets', () => {
  ...
  assert.match(homePage, /coffee-rewards\.webp/)
  assert.match(homePage, /vacation-beach-clean\.webp/)
  assert.match(homePage, /Membership Packages/)
  assert.match(homePage, /formatTenantCurrency\(1_000, program\)/)
  assert.match(homePage, /formatTenantCurrency\(4_000, program\)/)
  assert.match(homePage, /Coffee runs/)
...
runTest('platform guide is role-scoped and hidden from Loyality', () => {
  ...
  assert.match(guideSpec, /public guide replaces a legacy Tagalog preference with English/)
  ...
  assert.match(guideAuthSpec, /englishAdminOperationsLabels/)
  assert.match(guideAuthSpec, /englishBusinessStaffLabels/)
...
runTest('app version is installable as a PWA with online-required messaging', () => {
  ...
  const installPromptVisibility = readFileSync('src/lib/install-prompt-visibility.ts', 'utf8')
  ...
  assert.match(viteConfig, /strategies: 'injectManifest'/)
  assert.match(viteConfig, /filename: 'sw\.js'/)
  assert.match(viteConfig, /injectionPoint: undefined/)
  assert.match(main, /registerSW/)
  assert.match(app, /<OfflineNotice \/>/)
  assert.match(router, /<AppInstallPrompt \/>/)
  ...
  assert.match(installPrompt, /useLocation/)
  assert.match(installPrompt, /localStorage/)
  assert.match(installPromptVisibility, /INSTALL_PROMPT_ELIGIBLE_PATHS/)
  assert.match(installPromptVisibility, /'\/landing-page'/)
...
runTest('native app wrapper is configured for Android and iOS distribution', () => {
  ...
  assert.match(router, /url\.protocol === 'medellinrewards:' \|\| url\.protocol === 'rewardsplatform:'/)
  ...
  assert.match(androidManifest, /android:scheme="rewardsplatform"/)
  assert.match(androidStrings, /<string name="custom_url_scheme">rewardsplatform<\/string>/)
  assert.match(iosInfo, /CFBundleURLTypes/)
  assert.match(iosInfo, /<string>rewardsplatform<\/string>/)
...
runTest('all account roles use the approved unified auth portal', () => {
  ...
  assert.match(authPage, /signInAutomatically/)
  assert.match(authPage, /getHomePathForRole\(profile\.role\)/)
  assert.doesNotMatch(authPage, /SIGN_IN_PORTALS\.map/)
  assert.match(router, /LegacyPortalEntryRoute portal="business"/)
  assert.match(router, /LegacyPortalEntryRoute portal="admin"/)
...
runTest('unified sign in page follows the compact responsive portal layout', () => {
  ...
  assert.doesNotMatch(authSection, /showQuickTestSignIn/)
  assert.doesNotMatch(authSection, /quickTestAccounts/)
  assert.doesNotMatch(authSection, /data-testid=\{`quick-sign-in-\$\{portal\.id\}`\}/)
  assert.match(authSection, /signInAutomatically/)
  assert.match(authSection, /navigate\(redirect \|\| getHomePathForRole\(profile\.role\)\)/)
  assert.doesNotMatch(authSection, /<RewardMeTestCredentials/)
  assert.match(authShell, /showUtilityControls \? \(/)
  assert.match(authSection, /\{program\.name\}/)
  assert.match(authSection, /t\('Sign In'\)\.toUpperCase\(\)/)
  assert.doesNotMatch(authSection, /data-testid=\{`sign-in-portal-/)
  assert.doesNotMatch(authSection, /aria-pressed=\{isSelected\}/)
  ...
  assert.match(authSection, /aria-label=\{t\('Sign in'\)\}/)
  assert.match(authSection, /t\('Enter your email and password\. We will open the workspace assigned to your account\.'\)/)
  assert.match(authSection, /t\("Don't have an account\?"\)/)
  assert.match(authSection, /t\('Join \{program\}', \{ program: program\.name \}\)/)
...
runTest('early access page uses tenant-scoped preferences and exposes only English and Spanish', () => {
  const earlyAccessPage = readFileSync('src/features/early-access/pages/early-access-page.tsx', 'utf8')
  const language = readFileSync('src/lib/language.tsx', 'utf8')
  const languagePicker = readFileSync('src/components/language-picker.tsx', 'utf8')

  assert.match(language, /if \(typeof window === 'undefined'\) return 'en'/)
  assert.match(language, /tenantStorageKey\('language'\)/)
  assert.match(language, /if \(stored === 'en' \|\| stored === 'es'\) return stored/)
  assert.doesNotMatch(language, /if \(stored === 'en' \|\| stored === 'es' \|\| stored === 'tl'\)/)
  assert.match(languagePicker, /\['en', 'es'\] as Language\[\]/)
  assert.doesNotMatch(languagePicker, /\['en', 'tl', 'es'\]/)
  ...
  assert.match(language, /baseLanguage === 'es' \? 'es' : 'en'/)
  assert.match(language, /nextLanguage === 'es' \? 'es' : 'en'/)
...
runTest('Rewards Platform is isolated to platform-admin surfaces', () => {
  ...
  const adminLayout = readFileSync('src/layouts/admin-layout.tsx', 'utf8')
  const unifiedLogin = readFileSync('src/features/auth/pages/landing-page.tsx', 'utf8')
  const bootstrap = readFileSync('public/tenant-bootstrap.js', 'utf8')
  ...
  assert.doesNotMatch(unifiedLogin, /platformBrand\.name/)
  assert.match(unifiedLogin, /\{program\.name\}/)
...
runTest('business dashboard keeps transaction scanning out of the dashboard tab', () => {
  ...
  const imageScanner = readFileSync('src/lib/qr-image-scanner.ts', 'utf8')
  ...
  assert.match(transactionsPage, /Full phone screenshots work too\./)
  assert.match(transactionsPage, /data-customer-picker/)
  assert.match(transactionsPage, /useBusinessMembers/)
  assert.match(transactionsPage, /Search customers/)
...
runTest('customer shop route is a partner business map', () => {
  ...
  const mapLayout = readFileSync('src/features/shop/business-map-layout.ts', 'utf8')
  const customerLayout = readFileSync('src/layouts/customer-layout.tsx', 'utf8')

  assert.match(router, /path: '\/shop'[\s\S]*<ShopPage \/>/)
  assert.match(shopPage, /Partner Map/)
  assert.match(shopPage, /Explore Businesses/)
  assert.match(shopPage, /PartnerMapBackdrop/)
  assert.match(shopPage, /realistic-map-cartography/)
  assert.match(shopPage, /getBusinessMapPositions/)
  assert.match(shopPage, /hasExactMapPin/)
  assert.match(mapLayout, /PREVIEW_PIN_POSITIONS/)
  assert.match(mapLayout, /projectCoordinate/)
  assert.match(mapLayout, /findReadablePosition/)
...
runTest('business transactions page shows transaction history with optional gift card usage', () => {
  ...
  assert.match(redemptionsPage, /Complete Sale/)
  assert.match(redemptionsPage, /Redeem Gift Card and Complete Sale/)
  ...
  assert.match(redemptionsPage, /Full phone screenshots work too\./)
  ...
  assert.match(redemptionsPage, /Customer pays/)
  assert.match(redemptionsPage, /Customer earns/)
  assert.match(redemptionsPage, /Gift card covers/)
...
runTest('gift cards use declining balances and can be issued by businesses or admins', () => {
  const migration = readFileSync('supabase/migrations/20260709000000_declining_balance_gift_cards.sql', 'utf8')
  const domain = readFileSync('src/types/domain.ts', 'utf8')
  const service = readFileSync('src/integrations/supabase/services/gift-cards-service.ts', 'utf8')
  const hooks = readFileSync('src/features/gift-cards/hooks/use-gift-cards.ts', 'utf8')
  const redemptionsPage = readFileSync('src/features/gift-cards/pages/redemptions-page.tsx', 'utf8')
  const confirmationDialog = readFileSync('src/features/gift-cards/components/redemption-confirmation-dialog.tsx', 'utf8')
  const businessGiftCardsPage = readFileSync('src/features/gift-cards/pages/business-gift-cards-page.tsx', 'utf8')
  const adminGiftCardsPage = readFileSync('src/features/gift-cards/pages/admin-gift-cards-page.tsx', 'utf8')
  const display = readFileSync('src/features/gift-cards/components/gift-card-display.tsx', 'utf8')

  assert.match(migration, /add column if not exists initial_balance numeric\(12,2\)/)
  assert.match(migration, /add column if not exists remaining_balance numeric\(12,2\)/)
  assert.match(migration, /remaining_balance_after_value := round\(greatest\(available_balance_value - gift_card_amount_value, 0\)::numeric, 2\)/)
  assert.match(migration, /when remaining_balance_after_value <= 0 then 'redeemed'::public\.gift_card_status/)
  assert.match(migration, /else 'active'::public\.gift_card_status/)
  assert.match(migration, /'remaining_balance_after', remaining_balance_after_value/)
  assert.match(migration, /Gift card remaining balance: %s/)
  assert.match(migration, /actor_profile\.role in \('business-owner', 'business-staff'\)/)
  assert.match(migration, /actor_profile\.role = 'customer'/)
  assert.match(migration, /points_spent_value := catalog_row\.points_cost/)
  assert.match(migration, /points_spent_value integer := 0/)
  ...
  assert.match(redemptionsPage, /Balance after this sale:/)
  assert.match(redemptionsPage, /Remaining balance:/)
  assert.match(confirmationDialog, /Charge gift card/)
  assert.match(confirmationDialog, /stay active if a balance remains/)
  assert.match(businessGiftCardsPage, /useIssueGiftCardToCustomer/)
  assert.match(businessGiftCardsPage, /Give a customer a gift card/)
  assert.match(businessGiftCardsPage, /Business-issued cards start with the catalog value/)
  assert.match(adminGiftCardsPage, /useIssueGiftCardToCustomer/)
...
runTest('public business page follows the supplied local-partner reference', () => {
...
runTest('all credentials share one compact sign-in form and route by the authenticated role', () => {
  const page = readFileSync('src/features/auth/pages/landing-page.tsx', 'utf8')
  const router = readFileSync('src/routes/router.tsx', 'utf8')

  assert.match(page, /signInAutomatically/)
  assert.match(page, /getHomePathForRole\(profile\.role\)/)
  assert.doesNotMatch(page, /SIGN_IN_PORTALS/)
  assert.match(page, /id="signin-email"/)
  assert.match(page, /id="signin-password"/)
  assert.match(page, /t\('Forgot password\?'\)/)
  assert.doesNotMatch(page, /aria-pressed=\{isSelected\}/)
  assert.match(router, /getUnifiedSignInPath\('admin'/)
  assert.match(router, /getUnifiedSignInPath\('business'/)
  assert.doesNotMatch(router, /StaffLoginPage/)
})
...
runTest('active tenant operations use RewardMe and always verify canonical production brands', () => {
  ...
  assert.deepEqual(rewardMe, {
    slug: 'pinas',
    status: 'pending',
    hostname: 'rewardme-prod.vercel.app',
    senderEmail: null,
    monitor: true,
  })
  ...
  for (const [hostname, tenantName] of [
    ['rewardme-prod.vercel.app', 'RewardMe'],
