...
runTest('member signup page uses the split member portal layout', () => {
  const joinPage = readFileSync('src/features/join/pages/join-rewards-page.tsx', 'utf8')
  const activeJoinStart = joinPage.indexOf('export function JoinRewardsPage')
  const activeJoin = joinPage.slice(activeJoinStart)

  assert.match(activeJoin, /t\('Member Access'\)/)
  assert.match(activeJoin, /t\('Create your member account\.'\)/)
  assert.match(activeJoin, /t\('create account\.'\)/)
  assert.match(activeJoin, /min-h-\[31rem\]/)
  assert.match(activeJoin, /lg:grid-cols-\[minmax\(0,1fr\)_minmax\(27rem,0\.72fr\)\]/)
  assert.match(activeJoin, /Create my account/)
  assert.doesNotMatch(activeJoin, /AuthPortalShell/)
  assert.doesNotMatch(activeJoin, /activeTab="signup"/)
  assert.doesNotMatch(activeJoin, /heroImage/)
  assert.doesNotMatch(activeJoin, /Spend \$X locally/)
  assert.doesNotMatch(activeJoin, /Why we verify members/)
  assert.doesNotMatch(activeJoin, /bg-\[#24150e\]/)
})
...
runTest('member auth pages use the approved portal layouts', () => {
  ...
  assert.match(authPage, /export function AuthPage\(\) {\s*return <LegacyAuthPage \/>/)
  assert.match(joinPage, /export function JoinRewardsPage\(\)/)
  assert.match(joinPage, /t\('create account\.'\)/)
})

runTest('member sign in page follows the staff portal split layout', () => {
  const authPage = readFileSync('src/features/auth/pages/landing-page.tsx', 'utf8')
  const language = readFileSync('src/lib/language.tsx', 'utf8')
  const authSectionStart = authPage.indexOf('export function LegacyAuthPage')
  const authSectionEnd = authPage.indexOf('export function CompactAuthPage')
  const authSection = authPage.slice(authSectionStart, authSectionEnd)

  assert.match(authSection, /t\('Member Access'\)/)
  assert.match(authSection, /t\('Sign in to manage your member rewards\.'\)/)
  assert.match(authSection, /t\('Private member access'\)/)
  assert.match(authSection, /t\('Member portal'\)/)
  assert.match(authSection, /t\('sign in\.'\)/)
  assert.match(authSection, /Track your rewards, gift-card value/)
  assert.match(authSection, /min-h-\[31rem\]/)
  assert.match(authSection, /lg:grid-cols-\[minmax\(0,1fr\)_minmax\(27rem,0\.72fr\)\]/)
  ...
  assert.match(authSection, /t\('Sign In'\)/)
  assert.match(authSection, /t\('Need a member account\?'\)/)
  assert.match(authSection, /t\('Join now'\)/)
  assert.match(language, /'Member Access': 'Acceso de miembro'/)
  assert.match(language, /'Private member access': 'Acceso privado de miembro'/)
  assert.match(language, /'Member portal': 'Portal de miembro'/)
  assert.match(language, /'Need a member account\?': 'Necesitas una cuenta de miembro\?'/)
  assert.doesNotMatch(authSection, /AuthPortalShell/)
  assert.doesNotMatch(authSection, /Eye/)
})

runTest('member signup page follows the staff portal split layout', () => {
  const joinPage = readFileSync('src/features/join/pages/join-rewards-page.tsx', 'utf8')
  const language = readFileSync('src/lib/language.tsx', 'utf8')
  const activeJoinStart = joinPage.indexOf('export function JoinRewardsPage')
  const activeJoin = joinPage.slice(activeJoinStart)
  ...
  assert.match(activeJoin, /t\('Member Access'\)/)
  assert.match(activeJoin, /t\('Create your member account\.'\)/)
  assert.match(activeJoin, /t\('Private member access'\)/)
  assert.match(activeJoin, /t\('Member portal'\)/)
  assert.match(activeJoin, /t\('create account\.'\)/)
  ...
  assert.match(language, /'Create your member account\.': 'Crea tu cuenta de miembro\.'/)
  assert.match(language, /'create account\.': 'crea cuenta\.'/)
