...
runTest('all account roles use the approved unified auth portal', () => {
  ...
  assert.match(authPage, /getRequestedRoleForPortal\(selectedPortal\)/)
...
runTest('unified sign in page follows the compact responsive portal layout', () => {
  ...
  assert.match(authSection, /aria-pressed=\{isSelected\}/)
  ...
  assert.match(authSection, /t\('Sign in as \{role\}'/)
...
runTest('Rewards Platform is isolated to platform-admin surfaces', () => {
  ...
  assert.match(unifiedLogin, /selectedPortal === 'admin' \? platformBrand\.name : program\.name/)
...
runTest('business and admin choices share the compact role-verified auth layout', () => {
  const page = readFileSync('src/features/auth/pages/landing-page.tsx', 'utf8')
  const router = readFileSync('src/routes/router.tsx', 'utf8')

  assert.match(page, /SIGN_IN_PORTALS/)
  assert.match(page, /getRequestedRoleForPortal\(selectedPortal\)/)
  assert.match(page, /usePlatformDocumentBrand\(selectedPortal === 'admin'\)/)
