...
begin
  ...
  if p_original_bill is not null then
    ...
    gift_card_amount_value := round(least(greatest(coalesce(p_gift_card_amount, 0), 0), total_before_gift_card_value)::numeric, 2);
  ...
  if original_bill_value is not null and original_bill_value > 0 then
    ...
    values (
      ...
      actor_id,
      format(
        'Gift card code: %s. Gift card value: %s. Original receipt total: %s. Bill after gift card: %s. Tax added: %s. Service charge added: %s. Total before gift card: %s. Final bill after gift card: %s.',
        updated_card.code,
