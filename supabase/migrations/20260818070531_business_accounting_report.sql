-- Business-owner accounting view over immutable gift-card redemption events.
-- Reimbursement values are estimates: commercial settlement remains governed
-- by the signed program/business agreement.
...
begin
  ...
  select
    ...
    case
      when redemption.points_spent > 0 or redemption.issuer_role = 'platform-admin'
        then redemption.gift_card_applied
      else 0::numeric
    end as reimbursement_estimate,
    case
      when redemption.points_spent > 0 or redemption.issuer_role = 'platform-admin' then 'estimated'
      when redemption.issuer_role in ('business-owner', 'business-staff') then 'not_applicable'
      else 'review_required'
