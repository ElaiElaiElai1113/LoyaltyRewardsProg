drop function if exists public.redeem_gift_card(uuid, uuid);
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'original_bill', p_original_bill,
      'gift_card_amount', p_gift_card_amount,
  ...
  if p_original_bill is not null then
    ...
    charge_multiplier := 1 + coalesce(business_row.tax_rate, 0) + case
    ...
    purchase_amount_value := case
      when charge_multiplier > 1 then round((original_bill_value / charge_multiplier)::numeric, 2)
      else original_bill_value
    end;
    ...
    values (
      ...
      actor_id,
      format(
        'Gift card code: %s. Gift card value: %s. Original receipt total: %s.',
        updated_card.code,
