...
create unique index if not exists orders_profile_client_request_id_key
  on public.orders (profile_id, client_request_id)
...
alter table public.redemptions
  add column if not exists pickup_window text not null default 'Now';

create unique index if not exists redemptions_profile_client_request_id_key
  on public.redemptions (profile_id, client_request_id)
  where client_request_id is not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'redemptions_pickup_window_check'
      and conrelid = 'public.redemptions'::regclass
  ) then
    alter table public.redemptions
      add constraint redemptions_pickup_window_check
      check (pickup_window in ('Now', 'Within 30 mins', 'Later today'));
  end if;
end;
$$;
...
drop policy if exists "Staff can update reward balances" on public.reward_balances;
...
revoke all on function public.consume_reward_credit(uuid) from public;
...
grant execute on function public.consume_reward_credit(uuid) to authenticated;
