-- COALESCE is SQL syntax and cannot be schema-qualified. Replace both signup
-- functions before any application release calls the new referral path.
...
as $$
declare
  v_program_id_text text := pg_catalog.btrim(coalesce(new.raw_user_meta_data ->> 'active_program_id', ''));
  v_referral_code text := pg_catalog.btrim(coalesce(new.raw_user_meta_data ->> 'referral_code', ''));
  v_referral_business_id_text text := pg_catalog.btrim(coalesce(new.raw_user_meta_data ->> 'referral_business_id', ''));
  v_partner_code text := pg_catalog.btrim(coalesce(new.raw_user_meta_data ->> 'partner_referral_code', ''));
  v_partner_business_id_text text := pg_catalog.btrim(coalesce(new.raw_user_meta_data ->> 'partner_business_id', ''));
  v_program_id uuid;
  v_business_id uuid;
begin
