...
`rewardme-commercial-owner-signoff.md` before manually activating Regular or
Gold benefits, rewards, referrals, partner settlement, gift cards, or savings.

Online payment processing is not part of RewardMe. The application must not
collect card numbers or payment credentials.
...
| Trial end | Move the account to Free at expiry unless the member requests Regular or Gold access and operations approves it. | Avoids surprise charges and an unusable expired-account dead end. | Owner + legal |
...
| Regular plan | USD 25 per month is a reference price. Access begins only after authorized staff confirms eligibility and records approval evidence. | Matches the pitch while keeping entitlement changes auditable. | Owner + tax + operations |
| Gold plan | USD 100 per year is a reference price. Access begins only after authorized staff confirms eligibility and records approval evidence. | Matches the pitch while keeping annual terms explicit. | Owner + tax + operations |
| Currency | Approve whether reference prices are accepted in USD or converted outside the app. State the final amount, currency, tax, receipt, and renewal terms before activation. | Prevents exchange-rate and off-platform fee ambiguity. | Owner + tax |
| Cancellation | Define the effective date, remaining access, refund exceptions, and staff evidence required for a manual cancellation. | Makes manual plan administration consistent and auditable. | Owner + legal + operations |
...
No approval means no live transaction or benefit activation. The public site may
describe the intended program with honest availability labels, but manual plan
activation, reward funding, referral payouts, partner settlement, savings
matching, and gift-card fulfillment must remain unavailable until each exact gate
is signed and tested.
